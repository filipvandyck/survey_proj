...
def make_tsa_projectindex(project,file_planning):
    ...
    df_pr_project_signed = df_pr_project[df_pr_project['TSA_SIGNED']>pd.to_datetime('1900-1-1')]
    ...
    df_pl_signed = df_pl[df_pl['Datum_TSA_Getekend']>pd.to_datetime('1900-1-1')]
    ...
    df_pl_pr['TSA_DELTA'] = np.where(df_pl_pr['TSA_SIGNED']!=df_pl_pr['Datum_TSA_Getekend'], 'yes', 'no')
    ...
    df_pl_pr_delta = df_pl_pr_delta_full.rename(columns={"Opdrachtnummer_PR": "BUILDING KEY", "TSA_SIGNED": "TSA SIGNED Planrrr", "Datum_TSA_Getekend": "TSA SIGNED"})

        
    df_pl_pr_delta = df_pl_pr_delta[['BUILDING KEY','BG_KEY','Straat','Huisnummer','Huisnummer_Toevoeging','Quadrant','TSA SIGNED', 'TSA SIGNED Planrrr']]
