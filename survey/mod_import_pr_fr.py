...
INPUT_STREET_FILE_FR = os.path.dirname(os.path.realpath(__file__)) + '/exports/streets/streets_fr.csv' 
INPUT_CITY_FILE_FR = os.path.dirname(os.path.realpath(__file__)) + '/exports/streets/cities_fr.csv' 


testfile = INPUTFOLDER + 'test.csv'
...
def make_survey_import(file1, outputfolder, streetfile, cityfile, output_to_area=False):
    ...
    survey = pd.read_csv(file1, delimiter=';', encoding = ENCODING, keep_default_na=False)
    ...
    input_streets = pd.read_csv(streetfile, delimiter=';', encoding = ENCODING)
    ...
    input_cities = pd.read_csv(cityfile, delimiter=';', encoding = ENCODING, keep_default_na=False)
    ...
    if outputfolder != '':
        ...
        csv = survey.to_csv(exportfile,sep=';',index=False, encoding = ENCODING)
...
def make_street_names(file1,streetfile):
    ...
    survey = pd.read_csv(file1, delimiter=';', encoding = ENCODING)
    input_streets = pd.read_csv(streetfile, delimiter=';', encoding = ENCODING)
 ...
 #   columnReplaceAllChars(survey, 'Street')
    ...
    csv = out_streets.to_csv(streetfile,sep=';',columns=['id','Zip','LAM_Street_Code','Street','Area_Name'],index=False, encoding = ENCODING)
...
def make_city_names(file1,cityfile):
    ...
    csv = out_cities.to_csv(cityfile,sep=';',columns=['Zip','Municipality','CONTACT','EMAIL','TELNR'],index=False, encoding = ENCODING)
...
#make_import_streets_cities_file(testfile,INPUT_STREET_FILE_FR,INPUT_CITY_FILE_FR)
#make_import_pr_file(testfile,OUTPUTFOLDER,INPUT_STREET_FILE_FR,INPUT_CITY_FILE_FR)
